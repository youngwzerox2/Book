...
const FirebaseRegister = ({ ...others }) => {
  ...
  const register = () => {
    ...
    if (checked1 == false) {
      ...
      axios.post('/member/register', null, { params: testing });
    }
  };
    ...
    <>
      ...
      <Formik
        ...
        validationSchema={Yup.object().shape({
          email: Yup.string().email('유효한 이메일 주소를 입력해주세요').max(255).required('이메일을 입력해주세요'),
          password: Yup.string().max(255).required('비밀번호를 입력해주세요'),
          mname: Yup.string().max(255).required('이름을 입력해주세요')
        })}
      ...
      >
        {({ errors, handleBlur, handleChange, handleSubmit, touched, values }) => (
          <form noValidate onSubmit={handleSubmit} {...others}>
            <Grid container spacing={matchDownSM ? 0 : 2}></Grid>
            ...
            {strength !== 0 && (
              <FormControl fullWidth>
                <Box sx={{ mb: 2 }}>
                  <Grid container spacing={2} alignItems="center">
                    <Grid item>
                      <Button onClick={checkId}>이메일 인증하기</Button>
                    </Grid>
                  </Grid>
                </Box>
              </FormControl>
            ...
            <Grid container alignItems="center" justifyContent="space-between">
              <Grid item>
                <FormControlLabel
                  control={
                    <Checkbox checked={checked1} onChange={(event) => setChecked1(event.target.checked)} name="checked" color="primary" />
                  }
                  label={
